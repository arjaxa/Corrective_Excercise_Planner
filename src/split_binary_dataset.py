...
SOURCE_DIR = ROOT_DIR / "data" / "binary_posture_images"
TRAIN_DIR = SOURCE_DIR / "train"
VAL_DIR = SOURCE_DIR / "validation"

CLASSES = ["normal", "postural_deviation"]
...
if TRAIN_DIR.exists() or VAL_DIR.exists():
    existing_files = []
    if TRAIN_DIR.exists():
        existing_files += list(TRAIN_DIR.rglob("*"))
    if VAL_DIR.exists():
        existing_files += list(VAL_DIR.rglob("*"))
